...
class STViG(nn.Module):
    ...
    def forward(self, inputs, kpts=None):
        ...
        # CHANGED : return per frame prob
        x = nn.functional.adaptive_avg_pool2d(x, (T, 1))  # Keep time dimension
        x = self.fc(x).squeeze(-1).squeeze(-1)  # Remove only spatial dimensions
        return torch.sigmoid(x)  # Shape: (B, T) 
